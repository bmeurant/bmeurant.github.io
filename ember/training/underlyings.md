...
1. Créer un fichier html mettant en place un contexte Ember simple :
   ...
   <html>
     <head>
       ...
       <script src="http://builds.emberjs.com/tags/v2.9.0/ember.debug.js"></script>
       <script src="http://builds.emberjs.com/tags/v2.9.0/ember-template-compiler.js"></script>
     </head>
     <body>
...
La documentation est très complète sur le sujet et il n'est nul besoin de la paraphraser ici, je vous invite donc à vous y reporter [ici](http://guides.emberjs.com/v2.9.0/object-model/observers/).
...
* Cela signifie que plusieurs modifications déclencheront plusieurs fois les observeurs de manière non optimisée. Si l'on souhaite maîtriser d'avantage ces déclenchements, il est nécessaire de
  faire appel à la méthode ``Ember.run.once`` comme expliqué dans la [documentation](http://guides.emberjs.com/v2.9.0/object-model/observers/)
...
Cette API est décrite de manière succinte [ici](http://guides.emberjs.com/v2.9.0/object-model/enumerables/) et exhaustive [ici](http://emberjs.com/api/classes/Ember.Enumerable.html).
...
Pour reprendre l'exemple de la [doc officielle](http://guides.emberjs.com/v2.9.0/applications/run-loop/), si l'on a l'objet suivant :
...
our aller plus loin, se référer à la [documentation officielle](http://guides.emberjs.com/v2.9.0/applications/run-loop) et à cette 

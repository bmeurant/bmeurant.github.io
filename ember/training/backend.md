...
1. Fournir un *serializer* personnalisé pour permettre à [Ember Data](https://guides.emberjs.com/v2.9.0/models/) de comprendre la réponse du serveur lors de l'accès à ``http://localhost:4200/comics``
...
1. On souhaite désormais récupérer les albums embarqués lorsque l'on récupère un comic
   ...
   * Configurer l'application pour faire en sorte qu'[Ember Data](https://guides.emberjs.com/v2.9.0/models/) ajoute ce paramètre à la requête, dans le cas
   ...
   * Configurer l'application pour qu'[Ember Data](https://guides.emberjs.com/v2.9.0/models/) récupère les albums comme des relations embarquées du modèle ``comic``
...
1. Retourner à la racine de l'application ``http://localhost:4200/comics`` puis sélectionner ``blacksad``
   ...
   comic, [Ember Data](https://guides.emberjs.com/v2.9.0/models/) détecte que l'on a déjà chargé le modèle, n'éxécute pas le *hook* ``model`` ni
  ...
  1. Enfin, un dernier warning doit apparaître dû au fait que la méthode [Ember Data](https://guides.emberjs.com/v2.9.0/models/) `queryRecord` attend un 
